...
class Onchain:
    ...
    def safe_estimate_gas(self, tx: dict, min_gas: int = 150_000, max_attempts: int = 5) -> int:
        """
        Пытаемся безопасно оценить газ для транзакции.
        Если _estimate_gas падает, увеличиваем лимит на 10% до успеха или до max_attempts.
        """
        attempt = 0
        gas_limit = min_gas
        ...
        while attempt < max_attempts:
            try:
                ...
                # logger.warning(f'Попытка {attempt + 1}: оценка газа не удалась ({e}), увеличиваем запас...')
                gas_limit = int(gas_limit * 1.1)  # увеличиваем лимит на 10%
                tx['gas'] = gas_limit
                attempt += 1

        # logger.warning(f'Все попытки оценки газа не удались. Используем запас вручную: {gas_limit}')
        return gas_limit
