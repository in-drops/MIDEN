...
def time_filter(accounts: list[Account]) -> list[Account]:
    filter_accounts = []
    filter_limit = 9 # <= Минимальный лимит Faucets Public Note для аккаунтов
...
def activity(bot: Bot):
    ...
    while faucets < random_count and errors < 5:
        try:
            ...
            random_sleep(1, 3)
            request_button = bot.ads.page.locator('button[id="send-public-button"]')
